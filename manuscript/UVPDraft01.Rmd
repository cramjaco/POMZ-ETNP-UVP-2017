...
# Author list (putative, order not set, please suggest any others)
Jacob Cram
Jessica Pretty
Megan Duffy
Rachael L
...
* A
  + The biological pump is a key part of the global carbon cycle, in which, sinking particles transport carbon from the surface into the deep ocean. (Turner 2015; Neuer 2014)
  + Transfer efficiency, the flux between the base of the photic zone and the deep ocean (Francois 2002) is  important in particular, because the depth to which carbon is transported may affect global atmospheric carbon levels (Kwon)
  + Zooplankton can affect particle flux multiple several ways (Turner 2015; Steinberg and Landry 2017), and by extension the efficincy of the biogical pump (Cavan 2017).
    + (1) They repackages particles into fecal pellets which have different properties from the original particls Steinberg and Landry)
    ...
    - (4) Zooplankton may large partiles into smaller ones, through sloppy feeding (Cavin), or by generating turbulance that beraks downs particles (in Turner 2015 ->  Goldthwait et al. 2004), or part of a compex "gardening" (Mayor 2014).
   ...
   + Recently, Zooplankton transport in a week OMZ has been examined, and the zooplankton appeared to respond (somehow) to oxygen. It was postuated that in more anoxic regions zooplankton might not migrate as deep. (Kiko B).


   + Measurements of particle size and flux hold promise in further constraining models.
   + Most OMZ are in the oligotrophic ocean, where productivity is low. {Put somewhere}
...
In the above equation $\Delta Z$ is the distance, in meters between the current depth, and the depth for which the particle size distribution and flux change since, inform the model.
