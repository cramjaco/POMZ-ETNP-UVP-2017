...
Figure 4. Summary statistics of particle distributions for the deepest cast of the ETNP and non OMZ station. (A) Total particle numbers, (B) Particle size distribution. Color indicates the time point at which the sample were collected. As we did not detect appreciable differences between the stations, we report the color map for which points connect to which stations in the supplement.
...
[made, not added, ugly because flux is the same as number with low alpha -- consider changing alpha and ag]

Figure 6. Binned particle number (A), slope (B), and flux (C) for large ($>= 500\, \mu m$) and small ($< 500 \, \mu m$) particles, at both stations.
