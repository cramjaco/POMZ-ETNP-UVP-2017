...
## We hypothesized
 * Temporal day to day variability in paticle size distribution would be evident.
 ...
 * That there might also be temporal varibility in flux.
...
Figure 8. Depth binned particle number (volume normalized), particle size slope (psd), and flux (estimated as in Fig. 4) for large ($>= 500\, \mu m$), small ($< 500 \, \mu m$) and total particles, at both stations.
...
![](figures/stationP2_EK60_18kOnly.svg)

Figure 10. Acoustic data, measured by EK60, measured over the course of the experiment. Shown are data from the XX frequency band, which have highest depth penetration, but which appear to co-occur with data from other frequency bands (see Figure SX). Values are in return signal intensity and have not been normalized to observed biomass.
