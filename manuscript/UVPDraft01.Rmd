...
* A
  ...
  + Oxygen levels, and in particular anoxic regions of the water column, appear to modulate particle flux through the mesopelegic
   ...
   + Particle size resolved models of particle transfer through the mesopelegic oceans suggest that temperature, size and oxygen play a big role in modulating particle flux [@cramRoleParticleSize2018; @devriesExportFateOrganic2017].
     ...
     -They therefore predict that small particles will attenuate with depth.
     - However small particles have been shown to persist in deep waters (REF) and contribute substantialy to flux in the deep ocean (REF).
     - Fortunately, these models particle size predictions generate a useful null hypothesis of expexted particle size distributions in the abscence of zooplankton effects.
   ...
   + A recent study combined particle size tracking, mockness tows, acoustic data, and trap measurements from nearby locations found in literature to explore zooplankton transport in a weak OMZ [@kikoZooplanktonMediatedFluxesEastern2020].
     ...
     - The authors suggested that in stronger and larger OMZs there might be less active transport into the mesopelegic than seen at their site, since migratory organisms would presumably not migrate as deeply into the water and would be less active in true OMZs.
...
* B
  * The particle size data to test each of the three the Weber-Bianchi [-@weberEfficientParticleTransfer2020] models, id not exist prior to this study for fully anoxic OMZs of the pelegic ocean, to our knowledge.

  * Furthermore, combined measurements of particle numbers, flux and migration of plankton, while avalable in the hypoxic ETNA (Kiko), have not been made in a fully anoxic region.
    - Indeed this study used trap data from earlier studies. UVP and traps have been sampled together [@guidiRelationshipParticleSize2008], but not before in an OMZ.
  * While organic mater transport by zooplankton has been hypothesized to be limited in OMZs, this pattern hasn't been tested.
  * The degree to which zooplankton swimming or other processes lead to disaggregation in OMZs and elsewhere in the ocean is unknown.
...
* T
  ...
  * We collected particle size data at high temporal resolution over the course of a week
  * We integrate this size data with observed flux measurements, and acoustic data.
  * We describe the particle size distribution slope and how it changes with depth.
 ...
 * Do our data support any of the three Weber-Bianchi [-@weberEfficientParticleTransfer2020] models?
...
## We hypothesized
 * (**H1**) Temporal day to day variability in particle number, particle size distribution slope and flux would be evident.
 * (**H2**) This variability would relate to the location of migratory zooplankton, with a combination of increased particle flux and disaggregation present where zooplankton occur.
 * (**H3**) Disaggregation and particle production by zooplankton might lead to particle size patterns that cannot be explained by remineralization and sinking alone.
 * We also will test each of the three Weber-Bianchi models, specifically that OMZs have (**HWB1**) slower attenuation of all particles, (**HWB2**) decreased disaggregation, or (**HWB3**) slower attenuation of just large particles, in the OMZ core.
