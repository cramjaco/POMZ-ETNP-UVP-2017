...
```{r}
loc_DFP <- bds %>% filter(profile == "stn_043", depth %in% c(275)) %>% pull(DFP)
loc_use_DFP <- bds %>% filter(profile == "stn_043", depth %in% c(275)) %>% pull(use_this_DFP)
for_single_disag <- bes %>% filter(profile == "stn_043", depth %in% c(225, 275)) %>% select(depth, lb, nnp_smooth) %>% mutate(depth = recode(depth, `225` = "Shallow", `275` = "Deep")) %>%
