...
PlotPSD <- uds %>% 
  ...
  geom_point(alpha = .7, shape = 1, size = 2, stroke = 1, fill = "white") +
scale_y_reverse()
...
```{r}
bds %>% 
  ggplot(aes(y = depth, x = Flux_Smooth, col = profile)) + facet_wrap(~project) + geom_point() + scale_y_reverse(limits = c(2500, 0)) + scale_x_log10()
```
...
```{r}
bds %>% #filter(profile %in% c("stn_043", "p16n_100")) %>%
  ggplot(aes(y = depth, x = DFP, col = profile)) + facet_wrap(~project) + geom_point() + scale_y_reverse(limits = c(2500, 0)) + geom_vline(xintercept = 1)
```
...
```{r}
bds %>% 
  ggplot(aes(y = depth, x = ospsDZ, col = profile)) + facet_wrap(~project) + geom_point() + scale_y_reverse(limits = c(500, 0)) + geom_vline(xintercept = 0)
```
