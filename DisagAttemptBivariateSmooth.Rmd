...
```{r}
test_prev <- Dsmooth42_fixDFP %>% filter(depth == 125) %>% pull(spec_prev) %>% .[[1]]
test_spec <- Dsmooth42_fixDFP %>% filter(depth == 125) %>% pull(spec_only) %>% .[[1]]
test_DFP <- Dsmooth42_fixDFP %>% filter(depth == 125) %>% pull(DFP)
test_use_DFP <- Dsmooth42_fixDFP %>% filter(depth == 125) %>% pull(use_this_DFP)
test_spec
test_DFP
test_use_DFP
...
Dsmooth42_unnest %>% filter(depth == 125) %>% ungroup() %>% select(lb, nparticles, np_prev, np_pred) %>% pivot_longer(cols = c(nparticles, np_prev, np_pred)) %>% 
...
Dsmooth42_sr02_All %>% filter(DFP <= 1) %>%
  ggplot(aes(y = depth, x = Disag)) + geom_point() +
  scale_y_reverse()
```
