...
Dsmooth42_concise <- Dsmooth42_remineralized %>% 
  mutate(spec_meta = map2(spec_meta, spec_prev, ~tibble(.x, nnp_prev = .y))) %>%
  mutate(spec_meta = map2(spec_meta, spec_pred, ~tibble(.x, nnp_pred = .y))) %>%
...
Examine change
```{r}
Dsmooth42_unnest %>% filter(depth == 125) %>% ungroup() %>% select(lb, nnp, nnp_prev, nnp_pred) %>% pivot_longer(cols = c(nnp, nnp_prev, nnp_pred)) %>% 
...
Dsmooth42_sr02 <- Dsmooth42_unnest %>% 
  mutate(sizeFlux_prev = C_f * nnp_prev ^ ag,
         sizeFlux_pred = C_f * nnp_pred ^ ag)
Dsmooth42_sr02_Tot <- Dsmooth42_sr02 %>% group_by(depth) %>%
  summarize(DF = first(DF), DFP = first(DFP),
            Flux = sum(sizeFlux), Flux_Prev = sum(sizeFlux_prev), Flux_Pred = sum(sizeFlux_pred))
Dsmooth42_sr02_Small <- Dsmooth42_sr02 %>%
  filter(lb <= 0.53) %>%
  group_by(depth) %>%
  summarize(Flux = sum(sizeFlux), Flux_Prev = sum(sizeFlux_prev), Flux_Pred = sum(sizeFlux_pred))
...
Dsmooth42_sr02_All %>% filter(DFP <= 1) %>%
  ggplot(aes(y = depth, x = osps)) + geom_point() +
  scale_y_reverse()
```

Welp. These are suspiciously large.
